...
InfiValidator.prototype = {
    ...
    checkValues(location, params) {
        ...
        const _detectOpt = (validator, opts) => {
            let n = null;
            let _op = validator;
            let _opts = { ...opts };
            ...
            if (/:\w+$/.test(_op)) {
                [_op, n] = _op.split(':');
                _opts = _.assign({}, _opts, { arg: n });
            }

            return { arg: n, opName: _op, options: _opts }
        };
        ...
        for (const key of Object.keys(params)) {
            ...
            if (_.isString(ops)) {
                ...
                const { arg, opName, options } = _detectOpt(ops, self.options);

                if (_.has(self._validators.general, opName)) {
                    const result = self._validators.general[opName](this.mainObj[location][key], options);

                    if (!result) {
                        self[_privateAddError]({
                            rule: opName, field: key, location, val: arg
                        })
                    }
                } else {
                    logger.warn(`Validator of name '${opName}' does not exist.`);
            ...
            if (_.isArray(ops)) {
                for (const op of ops) {
                    ...
                    const { arg, opName, options } = _detectOpt(op, self.options);
                    ...
                    if (_.has(self._validators.general, opName)) {
                        ...
                        logger.warn(`[Validator] Validator of name '${opName}' does not exist.`);
                    }
                }
            }
        }
    ...
    cleanInjections() {
        ...
        for (const loc of ['body', 'params', 'query']) {
            ...
            if (isNoSQLInjected) {
                ...
                logger.warn(`NoSQL Injection detected in \'${loc}\' by user ${userInfo}`);
            }
    ...
    getSafeObject() {
        if (_.isNil(this._safeObj) || _.isEmpty(this._safeObj) || _.some(this._safeObj, _.isNil)) {
            logger.warn(`Cannot load safe object. Using original one now.`);
     ...
     * @return {void}
     */
    [_privateAddError]({ rule, field, location, code = 400, val = null }) {
        ...
        if (hasSecondPlaceholder && (val || location)) {
            msg = msg.replace('%2$', val ? val : location);
        }
